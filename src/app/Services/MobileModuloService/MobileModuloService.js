"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var core_1 = require("@angular/core");
var MobileModulo_1 = require("../../Classes/MobileModulo");
var MODULOS = [
    new MobileModulo_1.MobileModulo(1, 'Aprobaciones', true),
    new MobileModulo_1.MobileModulo(2, 'BI', true),
    new MobileModulo_1.MobileModulo(3, 'Facturación', true)
];
var RETRASO = 500;
var MobileModuloService = (function () {
    function MobileModuloService() {
    }
    MobileModuloService.prototype.getModulos = function () {
        return new Promise(function (data) {
            setTimeout(function () { return data(MODULOS); }, RETRASO);
        });
    };
    return MobileModuloService;
}());
MobileModuloService = __decorate([
    core_1.Injectable()
], MobileModuloService);
exports.MobileModuloService = MobileModuloService;
//# sourceMappingURL=MobileModuloService.js.map